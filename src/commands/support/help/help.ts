...
const command: ace.Command = {
    ...
    async execute(ctx) {
        // GATHER DATA
        const { getAllCommands, renderHelpView, resolveHelpTarget } = await import('@features/help/index.js');

        const all = await getAllCommands();
        const target = resolveHelpTarget(ctx.args, all);

        // LOGIC
        if (target.error === 'INVALID_COMMAND') {
            return ctx.error({
                embed: {
        ...
        if (target.error === 'INVALID_SUBCATEGORY') {
            return ctx.error({
                embed: {
        ...
        if (target.error === 'INVALID_CATEGORY') {
            return ctx.error({
                embed: {
